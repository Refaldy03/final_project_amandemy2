    ...
    <script>
        $(document).ready(function() {
            $('#searchForm').on('submit', function(e) {
                ...
                let kotaUmkm = $('#kotaUmkm').val();

                $.ajax({
                    type: "GET",
                    url: "{{ route('searchUmkm') }}",
                    data: {
                        kota_umkm: kotaUmkm
                    },
                    success: function(response) {
                        // Update umkmContainer dengan data baru
                        $('#umkmContainer').empty();
                        if (response.umkms.length > 0) {
                            $.each(response.umkms, function(index, umkm) {
                                let umkmCard = `<div class="col-sm-6 col-md-4 col-lg-3 mb-3">
                                <a href="{{ url('detailUmkm') }}/${umkm.id}" class="text-decoration-none">
                                <div class="card w-100 h-100" style="background-color: rgb(243, 198, 240)">
                                    <img class="card-img-top rounded img-fluid"
                                        style="object-fit: cover; height: 150px;"
                                        src="{{ asset('storage/umkm_images/') }}/${umkm.foto_umkm}"
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between my-auto" style="font-size: 18px">
                                            <p class="card-title text-dark fw-bold my-auto">${umkm.nama_umkm}</p>
                                        </div>
                                        <div class="d-flex justify-content-between flex-wrap mt-2">
                                            <div class="text-start mb-2 mb-md-1">
                                                <p class="my-auto rounded bg-dark text-white py-1 px-2 fw-semibold"
                                        ...
                                        <p style="font-size: 14px; overflow: hidden; text-overflow: ellipsis;
                                            display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical;
                                            margin: 10px auto;">
                                            ${umkm.deskripsi}
                                        </p>
                                        <a href="https://wa.me/${umkm.kontak}" class="btn btn-primary btn-sm w-100 fw-bold mt-auto">Hubungi Kontak <svg class="bg-success rounded-circle mb-1" width="20"
